...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/terminal/state.rs
...
// Amon hook integrations are hook-authoritative while live; screen recovery
...
impl TerminalState {
    ...
    fn route_full_lifecycle_hook_report(
        ...
        let opencode_cross_talk = (source, agent_label) == ("amon:opencode", "opencode")
            && process_present
    ...
    fn lifecycle_hook_report_replaces_persisted_session(
        ...
        self.hook_authority.is_none()
            && (source, agent_label) == ("amon:mastracode", "mastracode")
            && self
                .persisted_agent_session
                .as_ref()
                .is_some_and(|session| {
    ...
    fn session_report_allows_session_replacement(
        ...
        matches!(
            ...
            ) | ("amon:mastracode", "mastracode", Some("startup"))
                | ("amon:hermes", "hermes", Some("startup" | "new" | "resume"))
                | ("amon:opencode", "opencode", Some("select"))
                | ("amon:pi", "pi", Some("new" | "resume" | "fork"))
                | (
                    "amon:omp",
                    "omp",
    ...
    fn is_unsequenced_opencode_selection(
        ...
        (source, agent_label, session_start_source, seq)
            == ("amon:opencode", "opencode", Some("select"), None)
    }
...
mod tests {
    ...
    fn session_only_report_does_not_create_hook_authority() {
        for (agent, source, label, session_id) in [
            (Agent::Codex, "amon:codex", "codex", "codex-session"),
            (Agent::Devin, "amon:devin", "devin", "devin-session"),
        ] {
            let mut terminal = test_terminal();
    ...
    fn startup_session_claim_activates_full_lifecycle_integrations() {
        for (agent, source, label) in [
            (Agent::Kimi, "amon:kimi", "kimi"),
            (Agent::Kilo, "amon:kilo", "kilo"),
        ] {
            let mut terminal = test_terminal();
            terminal.set_detected_state(Some(agent), AgentState::Idle);
    ...
    fn opencode_tui_selection_anchors_after_process_detection() {
        ...
        assert!(!terminal.hook_report_sequences.contains_key("amon:opencode"));
    }
    ...
    fn opencode_tui_selection_reanchors_full_lifecycle_authority() {
        ...
        assert!(!terminal
            ...
            .contains_key("amon:opencode"));
        assert_eq!(
            terminal.hook_report_sequences.get("amon:opencode"),
            Some(&20)
        );
        assert_eq!(
            terminal
    ...
    fn different_owner_session_ref_does_not_replace_existing_session_ref() {
        ...
        assert_eq!(
            ...
            Some(("amon:droid", "droid", "droid-session"))
        );
    }
    ...
    fn foreground_agent_session_replaces_stale_different_owner_session_ref() {
        for session_start_source in ["resume", "startup"] {
            ...
            assert_eq!(
                ...
                Some(("amon:claude", "claude", "claude-session")),
    ...
    fn foreground_agent_session_requires_lifecycle_source_to_replace_different_owner() {
        for session_start_source in [None, Some("other")] {
            ...
            assert_eq!(
                ...
                Some(("amon:codex", "codex", "codex-session"))
            );
        }
    }
    ...
    fn clearing_hook_authority_clears_session_ref() {
        ...
        let mutation = terminal
            .clear_hook_authority_with_mutation(Some("amon:pi"), Some(21))
    ...
    fn agent_alias_survives_detection_uncertainty_and_reported_release_but_not_replacement() {
        ...
        let mutation = terminal
            .release_agent_with_mutation("amon:codex", "codex", None)
    ...
    fn release_agent_clears_matching_restored_session_ref_before_detection() {
        let mut terminal = test_terminal();
        terminal.set_persisted_agent_session(crate::agent_resume::PersistedAgentSession {
            source: "amon:hermes".into(),
            agent: "hermes".into(),
            session_ref: crate::agent_resume::AgentSessionRef::id("hermes-session").unwrap(),
        });

        let mutation = terminal
            .release_agent_with_mutation("amon:hermes", "hermes", Some(21))
    ...
    fn release_agent_preserves_foreign_persisted_session_ref() {
        ...
        let mutation = terminal
            .release_agent_with_mutation("amon:pi", "pi", Some(21))
