...
// Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/assets/opencode/herdr-agent-state.test.ts
...
beforeEach(() => {
  ...
  process.env.GAZE_SOCKET_PATH = "test.sock";
  process.env.GAZE_AGENT_ID = "test:p1";
});

async function loadPlugin() {
  importCounter += 1;
  const { HerdrAgentStatePlugin } = await import(`./gaze-agent-state.js?test=${importCounter}`);
...
test("reports retry status as working", async () => {
  ...
  expect(requests.map(requestMethod)).toEqual(["agent.report_state"]);
