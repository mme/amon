...
## Postscript: the session got the script after all

The body rejects a script on `PATH` and prices the alias at "interactive
shells only". ADR-0013 built the rejected mechanism anyway — for the half the
alias structurally cannot reach: Omarchy launches an agent through a
terminal's `-e`, which `exec`s the binary with no shell and therefore no
alias. `amon setup` now also writes a shim per set-up agent into
`~/.local/share/amon/shims/`, prepended to the session `PATH` by
`~/.config/hypr/amon.lua`.

The recursion this body calls disqualifying is terminated differently than
the dropped script did it: not by the shim resolving the real agent around
itself, but by the environment — a shim that sees `AMON_AGENT_ID` knows amon
is the caller and `exec`s the real binary. And the shim directory is amon's
own, so the `~/.local/bin` overwrite hazard named above never arises.

What survives of this decision: the alias remains the mechanism for shells,
for the reasons given — `PATH` lookup never consults aliases, so it cannot
recurse, and a missing amon fails loudly instead of silently unwrapping. What
is superseded is the consequence's price tag: "an agent started by a
keybinding … runs unwrapped" now holds only off Omarchy. On it, the
keybinding path wraps exactly the agents you set up, which is this ADR's
promise extended rather than a second policy — see ADR-0013 for the whole of
it.
