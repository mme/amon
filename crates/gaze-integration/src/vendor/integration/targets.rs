...
// Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/targets.rs
...
pub(crate) fn remove_legacy_pi_extension_from_omp_dir(dir: &Path) -> io::Result<bool> {
    ...
    if content.contains("GAZE_INTEGRATION_ID=pi") {
...
/// The complete Gaze-owned Grok hook config. Installation and status share
...
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    ...
    // Grok merges every `~/.grok/hooks/*.json`, so gaze owns a dedicated
...
pub(crate) fn uninstall_grok() -> io::Result<GrokUninstallResult> {
    ...
    // gaze owns both files outright, so removal is a straight delete.
