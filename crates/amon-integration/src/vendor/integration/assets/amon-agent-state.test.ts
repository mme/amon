...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/integration/assets/herdr-agent-state.test.ts
...
const originalEnvironment = {
  AMON_ENV: process.env.AMON_ENV,
  AMON_OMP_IDLE_DEBOUNCE_MS: process.env.AMON_OMP_IDLE_DEBOUNCE_MS,
  AMON_AGENT_ID: process.env.AMON_AGENT_ID,
  AMON_SOCKET_PATH: process.env.AMON_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/amon-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/amon-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/amon-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/amon-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  ...
  process.env.AMON_SOCKET_PATH = recordingSocketPath;
  process.env.AMON_AGENT_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `amon-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `amon-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
...
test("OpenCode stays disabled without the Amon socket environment", async () => {
  process.env.AMON_ENV = "1";
  process.env.AMON_AGENT_ID = "test:p1";
  delete process.env.AMON_SOCKET_PATH;

  const { HerdrAgentStatePlugin } = await importFresh("./opencode/amon-agent-state.js");
...
for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `amon-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
  ...
  test(`${integration.name} reload preserves working state when the agent is active`, async () => {
    ...
    const reportedState = () => {
      for (const request of requests) {
        if (!isRecord(request) || request.method !== "agent.report_state") {
          continue;
        }
...
test("OMP accepts POSIX and Windows session paths", async () => {
  const { isAbsoluteSessionPath } = await importFresh("./omp/amon-agent-state.ts");
...
test("Pi reports idle only after the agent settles", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/amon-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("amon:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("amon:blocked")?.({ active: false }, context);
...
test("Pi reports the session replacement source", async () => {
  ...
  const reportedSession = () =>
    requests.find((request) => isRecord(request) && request.method === "agent.report_session");
  const deadline = Date.now() + 1_000;
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `amon-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
  ...
  const recordingServer = createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      if (isRecord(request) && request.method === "agent.report_session") {
  ...
  expect(
    requests.some((request) => isRecord(request) && request.method === "agent.report_state"),
  ).toBe(false);
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.AMON_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/amon-agent-state.ts");
  install(pi);
...
function requestStates(requests: unknown[]): unknown[] {
  return requests
    .filter((request) => isRecord(request) && request.method === "agent.report_state")
    .map(requestState);
}
