...
const idleDebounceMs = parseDurationEnv("AMON_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("AMON_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("amon:blocked", (data) => {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
