...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/integration/targets.rs
...
pub(crate) fn remove_legacy_pi_extension_from_omp_dir(dir: &Path) -> io::Result<bool> {
    ...
    if content.contains("AMON_INTEGRATION_ID=pi") {
...
pub(crate) fn install_antigravity_cli() -> io::Result<AntigravityCliInstallPaths> {
    ...
    // The Amon block is Amon-owned, so rewrite it wholesale and leave every
...
/// Builds the Amon-owned `hooks.json` block for Antigravity CLI.
///
/// Every event Amon registers takes a flat handler list; the `matcher`/`hooks`
/// group is only valid for the tool events, which Amon does not use.
...
/// The complete Amon-owned Grok hook config. Installation and status share
...
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    ...
    // Grok merges every `~/.grok/hooks/*.json`, so amon owns a dedicated
...
pub(crate) fn uninstall_grok() -> io::Result<GrokUninstallResult> {
    ...
    // amon owns both files outright, so removal is a straight delete.
