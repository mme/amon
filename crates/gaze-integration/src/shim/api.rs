...
pub const SOCKET_PATH_ENV_VAR: &str = gaze_protocol::env::SOCKET_PATH;

/// herdr points spawned agents at its daemon socket. gaze points them at the
...
/// `apply_pane_base_env`, which gaze does not use.
...
pub mod schema {
    ...
    /// Every agent gaze can install an integration for. Kept in herdr's order
