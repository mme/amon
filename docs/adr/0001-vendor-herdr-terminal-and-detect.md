...
Amon detects agent state (Idle/Working/Blocked/Unknown) by evaluating herdr's
...
So amon splits the difference. Emulation comes from the `libghostty-vt` crate
...
spawn and manage multiplexer panes, and amon spawns its own PTY. That cut
...
(`$XDG_RUNTIME_DIR/amon/agents/<id>.sock`, unlinked on exit) and injects
`AMON_ENV=1`, `AMON_SOCKET_PATH=<that socket>`, `AMON_AGENT_ID=<registry id>`
...
hook filenames also let amon and herdr coexist on one machine.
...
Building amon requires Zig 0.15.2, which `cargo install` users must have.
...
amon also does not vendor herdr's per-platform process scanning: it identifies
