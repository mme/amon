...
`gaze install <agent>` / `gaze uninstall <agent>` / `gaze integrations` are
...
All 16 targets are ported (env vars renamed `HERDR_*` → `GAZE_*`, socket
methods renamed to the gaze protocol); claude, codex, and pi are smoke-tested
...
`crates/gaze-integration`. Modifications (renames, protocol methods) are noted
