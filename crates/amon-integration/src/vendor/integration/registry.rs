...
// Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/registry.rs
...
pub(crate) fn print_outdated_update_notice() -> bool {
    ...
    let targets = outdated
        ...
        .collect::<Vec<_>>();
    eprintln!(
        "installed amon integrations need updating; {}.",
        integration_update_instructions(&targets).replace('`', "")
    );
    true
}

/// Whether the Amon-owned Grok hook config exactly matches the installed
...
pub(crate) fn integration_status_at(
    ...
    // Grok only invokes the hook when the amon-owned `hooks/amon.json`
    // registers it, so a current hook script with a missing or broken config
    // is a nonfunctional install: report it as outdated so `amon integration
