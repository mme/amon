...
// are the only Narration omp has. herdr's own extension beside this one keeps
// reporting state and session identity; this file reports content and nothing
// else. Installed and removed by amon-integration/src/activity_hooks/omp.rs.
//
// installed by amon
// managed by amon; `amon remove omp` deletes it.
// AMON_OMP_ACTIVITY_VERSION=1

import * as net from "node:net";

const source = "amon:omp-activity";
...
export default function (pi: any) {
  ...
  // Only the root UI session reports, the same gate herdr's own omp extension
  // uses — omp's fork predates pi's `mode` field, so `hasUI` is the test, and
  // activation is lazy: a reload can replace this extension mid-run without
  // another session_start.
