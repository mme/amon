...
fn methods_are_named_as_the_hooks_send_them() {
    ...
    assert_eq!(
        Method::AgentReportState(gaze_protocol::ReportState {
            agent_id: "a1".into(),
            source: "gaze:claude".into(),
            agent: "claude".into(),
            state: AgentState::Working,
            seq: 1,
            agent_session_id: None,
...
fn unknown_events_are_skipped_not_fatal() {
    // A subscriber built against an older gaze meets an event from a newer one.
...
fn error_responses_carry_a_code_and_a_message() {
    let response = Response::err(
        "req-7",
        gaze_protocol::Error::new(ErrorCode::UnknownAgent, "no such agent: a9"),
    );
...
fn a_patch_changes_only_what_it_names() {
    ...
    assert_eq!(entry.title.as_deref(), Some("gaze.sh"), "title untouched");
