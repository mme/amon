//! The one thread that owns what gaze knows about the agent.
//!
...
use gaze_detect::{detect_agent_with_osc, Agent};
use gaze_protocol::{AgentPatch, AgentState};
use gaze_term::{ShadowTerminal, TerminalId, TerminalState};
...
pub fn spawn(
    ...
    std::thread::spawn(
        move || match Observer::new(agent_id, agent, cwd, cols, rows, link) {
            ...
            Err(error) => {
                eprintln!("gaze: shadow terminal unavailable, state will not be reported: {error}");
            }
        },
    )
}
...
fn to_detect_state(state: AgentState) -> gaze_detect::AgentState {
    match state {
        AgentState::Idle => gaze_detect::AgentState::Idle,
        AgentState::Working => gaze_detect::AgentState::Working,
        AgentState::Blocked => gaze_detect::AgentState::Blocked,
        AgentState::Unknown => gaze_detect::AgentState::Unknown,
    }
}

fn from_detect_state(state: gaze_detect::AgentState) -> AgentState {
    match state {
        gaze_detect::AgentState::Idle => AgentState::Idle,
        gaze_detect::AgentState::Working => AgentState::Working,
        gaze_detect::AgentState::Blocked => AgentState::Blocked,
        gaze_detect::AgentState::Unknown => AgentState::Unknown,
    }
}
