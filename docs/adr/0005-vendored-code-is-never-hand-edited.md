...
Gaze vendors three substantial herdr modules (ADR-0001, ADR-0004) and wants to
...
1. **Import paths are not edited.** Each gaze crate recreates the
...
2. **The herdr → gaze rename is a deterministic token map** applied while
   copying upstream into `crates/`: `HERDR_`→`GAZE_`, `herdr`→`gaze` in
   ...
   let gaze and herdr be installed on the same machine.
