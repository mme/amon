...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/agent_resume.rs
...
pub fn is_reserved_native_state_source(source: &str, agent: &str) -> bool {
    matches!(
        ...
        ("amon:claude", "claude")
            | ("amon:codex", "codex")
            | ("amon:copilot", "copilot")
            | ("amon:devin", "devin")
            | ("amon:droid", "droid")
            | ("amon:qodercli", "qodercli")
            | ("amon:qwen", "qwen")
            | ("amon:cursor", "cursor")
            | ("amon:grok", "grok")
    )
...
pub fn plan(source: &str, agent: &str, session_ref: &AgentSessionRef) -> Option<AgentResumePlan> {
    ...
    let argv = match (source, agent, session_ref.kind) {
        ("amon:claude", "claude", AgentSessionRefKind::Id) => {
            vec![
                "claude".into(),
                "--resume".into(),
        ...
        ("amon:pi", "pi", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
            vec!["pi".into(), "--session".into(), session_ref.value.clone()]
        }
        ("amon:omp", "omp", AgentSessionRefKind::Path | AgentSessionRefKind::Id) => {
        ...
        ("amon:hermes", "hermes", AgentSessionRefKind::Id) => {
            vec![
                "hermes".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("amon:opencode", "opencode", AgentSessionRefKind::Id) => {
            vec![
                "opencode".into(),
                "--session".into(),
                session_ref.value.clone(),
        ...
        ("amon:qodercli", "qodercli", AgentSessionRefKind::Id) => {
            vec![
                "qodercli".into(),
                "--resume".into(),
                session_ref.value.clone(),
        ...
        ("amon:qwen", "qwen", AgentSessionRefKind::Id) => {
            vec!["qwen".into(), "--resume".into(), session_ref.value.clone()]
        }
        ("amon:kilo", "kilo", AgentSessionRefKind::Id) => {
            vec!["kilo".into(), "--session".into(), session_ref.value.clone()]
        }
        ("amon:cursor", "cursor", AgentSessionRefKind::Id) => {
            vec![
                if cfg!(windows) {
                    "cursor-agent.cmd"
                } else {
        ...
        ("amon:antigravity_cli", "agy", AgentSessionRefKind::Id) => {
            vec![
                "agy".into(),
                "--conversation".into(),
                session_ref.value.clone(),
        ...
        ("amon:grok", "grok", AgentSessionRefKind::Id) => {
...
pub(crate) fn is_official_agent_source(source: &str, agent: &str) -> bool {
    matches!(
        ...
        ("amon:claude", "claude")
            ...
            | ("amon:kimi", "kimi")
            | ("amon:omp", "omp")
            | ("amon:mastracode", "mastracode")
            | ("amon:pi", "pi")
            | ("amon:hermes", "hermes")
            | ("amon:opencode", "opencode")
            | ("amon:qodercli", "qodercli")
            | ("amon:qwen", "qwen")
            | ("amon:kilo", "kilo")
            | ("amon:cursor", "cursor")
            | ("amon:antigravity_cli", "agy")
            | ("amon:grok", "grok")
    )
}
...
mod tests {
    ...
    fn native_state_reservation_excludes_full_lifecycle_sources() {
        assert!(is_reserved_native_state_source("amon:claude", "claude"));
        assert!(is_reserved_native_state_source("amon:codex", "codex"));
        assert!(is_reserved_native_state_source("amon:devin", "devin"));
        assert!(!is_reserved_native_state_source("amon:kimi", "kimi"));
    ...
    fn report_ref_prefers_pi_and_omp_paths_and_validates_values() {
        ...
        let session_ref =
            session_ref_from_report("amon:omp", "omp", Some("omp-id".into()), None).unwrap();
        ...
        assert_eq!(session_ref.value, "omp-id");
        assert!(
            session_ref_from_report("amon:omp", "omp", None, Some("relative.jsonl".into()))
                .is_none()
        );

        assert!(
            session_ref_from_report("amon:claude", "claude", None, Some(claude_session)).is_none()
        );

        let session_ref =
            session_ref_from_report("amon:copilot", "copilot", Some("copilot-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "copilot-id");
        assert!(
            session_ref_from_report("amon:copilot", "copilot", None, Some(copilot_session))
                .is_none()
        );

        let session_ref =
            session_ref_from_report("amon:devin", "devin", Some("devin-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "devin-id");

        let session_ref =
            session_ref_from_report("amon:droid", "droid", Some("droid-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("amon:kimi", "kimi", Some("kimi-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        ...
        let session_ref =
            session_ref_from_report("amon:kilo", "kilo", Some("kilo-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "kilo-id");

        let session_ref =
            session_ref_from_report("amon:qodercli", "qodercli", Some("qoder-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "qoder-id");

        let session_ref =
            session_ref_from_report("amon:qwen", "qwen", Some("qwen-id".into()), None).unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
        assert_eq!(session_ref.value, "qwen-id");

        let session_ref =
            session_ref_from_report("amon:antigravity_cli", "agy", Some("agy-id".into()), None)
                .unwrap();
        assert_eq!(session_ref.kind, AgentSessionRefKind::Id);
