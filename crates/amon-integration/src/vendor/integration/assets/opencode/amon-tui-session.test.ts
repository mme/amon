...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/integration/assets/opencode/herdr-tui-session.test.ts
...
async function loadPlugin() {
  importCounter += 1;
  const module = await import(`./amon-tui-session.js?test=${importCounter}`);
...
test("retries an initial selection while Amon detects the process", async () => {
  const plugin = await loadPlugin();
  const tui = fakeApi();
  tui.addSession({ id: "session-a" });
  tui.select("session-a");
