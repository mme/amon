...
//! uninstall is removing it. The extension reports the submitted prompt as a
//! turn boundary and tool calls / the reply's first line as narration; pi's
//! screen expresses that structure through background colour alone, which
//! amon's text reader cannot see, so these events are the only Narration pi
//! has.
...
const ASSET: &str = include_str!("../assets/pi-activity.ts");
/// Mirrors the `AMON_PI_ACTIVITY_VERSION` stamp in the extension.
pub const VERSION: u32 = 1;

fn extension_path() -> io::Result<PathBuf> {
    Ok(pi_extension_dir()?.join(INSTALL_NAME))
}

/// Writes the extension. Idempotent. `None` when pi is not set up here.
pub fn install() -> io::Result<Option<String>> {
    let Ok(dir) = pi_extension_dir() else {
        return Ok(None);
    };
...
pub fn state() -> io::Result<HookState> {
    state_of(
        &extension_path()?,
        &format!("AMON_PI_ACTIVITY_VERSION={VERSION}"),
    )
}
...
mod tests {
    ...
    fn the_asset_stamps_the_version_state_checks_for() {
        assert!(
            ASSET.contains(&format!("AMON_PI_ACTIVITY_VERSION={VERSION}")),
