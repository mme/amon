...
// Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/assets/herdr-agent-state.test.ts
...
const originalEnvironment = {
  GAZE_ENV: process.env.GAZE_ENV,
  GAZE_OMP_IDLE_DEBOUNCE_MS: process.env.GAZE_OMP_IDLE_DEBOUNCE_MS,
  GAZE_AGENT_ID: process.env.GAZE_AGENT_ID,
  GAZE_SOCKET_PATH: process.env.GAZE_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/gaze-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/gaze-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/gaze-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/gaze-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  ...
  process.env.GAZE_SOCKET_PATH = recordingSocketPath;
  process.env.GAZE_AGENT_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `gaze-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `gaze-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
...
test("OpenCode stays disabled without the Gaze socket environment", async () => {
  process.env.GAZE_ENV = "1";
  process.env.GAZE_AGENT_ID = "test:p1";
  delete process.env.GAZE_SOCKET_PATH;

  const { HerdrAgentStatePlugin } = await importFresh("./opencode/gaze-agent-state.js");
...
for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `gaze-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
...
test("Pi reports idle only after the agent settles", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/gaze-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("gaze:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("gaze:blocked")?.({ active: false }, context);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `gaze-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.GAZE_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/gaze-agent-state.ts");
  install(pi);
