...
## Postscript: the shadowed names have moved

The consequence above lists the subcommands as they were. `install`,
`uninstall` and `integrations` became `setup`, `remove` and `doctor`
(ADR-0010), and `focus` joined (ADR-0011), so the names that shadow agents
today are `status`, `setup`, `remove`, `focus`, `doctor`, `daemon`, `hook`
and `help`. An agent named `focus` now needs the path form (`amon ./focus`);
one named `integrations` no longer does. The invariant itself is unchanged —
only the reserved words drift as amon grows, which is worth knowing exactly
because this consequence is where someone will look when a name collides.
