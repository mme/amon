//! `gaze` — run a coding agent behind a lens.
//!
//! Argument handling is deliberately hand-rolled rather than derived: `gaze
//! claude --help` must give the agent its `--help`, not gaze's, and everything
...
fn print_help() {
    println!(
        "gaze {VERSION} — run coding agents behind a lens

usage:
  gaze <agent> [args...]     wrap an agent; everything after it is the agent's
  gaze status [--json]       what every connected agent is doing
  gaze install <agent>       install this agent's integration hooks
  gaze uninstall <agent>     remove them
  gaze integrations          which integrations are installed, and how current
  gaze daemon                run the daemon in the foreground
...
fn run_agent(argv: &[String]) -> ExitCode {
    ...
    if let Some(notice) = gaze_integration::outdated_notice(&agent_name(&argv[0])) {
        eprintln!("{notice}");
    }

    match gaze_wrapper::run(gaze_wrapper::Launch {
        argv: argv.to_vec(),
...
fn run_daemon() -> Result<(), Box<dyn std::error::Error>> {
    gaze_daemon::run(VERSION.to_string())?;
    Ok(())
}
...
fn run_install(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for message in gaze_integration::install(target)? {
        println!("{message}");
    }
    Ok(())
}
...
fn run_uninstall(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for message in gaze_integration::uninstall(target)? {
        println!("{message}");
    }
    Ok(())
}
...
fn require_target(
    args: &[String],
    action: &str,
) -> Result<gaze_integration::IntegrationTarget, Box<dyn std::error::Error>> {
    let Some(name) = args.first() else {
        return Err(format!("usage: gaze {action} <agent>\n{}", known_agents()).into());
    };
    gaze_integration::parse_target(name)
        .ok_or_else(|| format!("unknown agent: {name}\n{}", known_agents()).into())
}

fn known_agents() -> String {
    let names: Vec<&str> = gaze_integration::all_targets()
        .into_iter()
        .map(gaze_integration::target_label)
        .collect();
    format!("known agents: {}", names.join(", "))
}

fn run_integrations() -> Result<(), Box<dyn std::error::Error>> {
    for status in gaze_integration::statuses() {
        let state = match status.state {
            gaze_integration::InstallState::NotInstalled => "not installed".to_string(),
            gaze_integration::InstallState::Current => {
                format!("current (v{})", status.expected_version)
            }
            gaze_integration::InstallState::Outdated => format!(
                "outdated (v{} < v{})",
                status
                    .installed_version
...
impl Client {
    fn connect_or_start() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Err("could not reach or start the gaze daemon".into())
    }
