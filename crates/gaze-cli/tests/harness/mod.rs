//! Support for driving the real `gaze` binary.
//!
...
pub const GAZE: &str = env!("CARGO_BIN_EXE_gaze");
...
impl Sandbox {
    ...
    pub fn command(&self, args: &[&str]) -> Command {
        let mut command = Command::new(GAZE);
        command.args(args);
    ...
    pub fn run(&self, args: &[&str]) -> std::process::Output {
        self.command(args).output().expect("gaze runs")
    }
    ...
    /// Polls `gaze status` until `predicate` accepts the agent list, and
    ...
    pub fn daemon_socket(&self) -> PathBuf {
        self.runtime.join("gaze/gazed.sock")
    }
    ...
    /// real agent is what makes gaze apply that agent's detection manifest.
...
/// Runs a program on a plain PTY, with no gaze involved, and returns exactly
/// what the terminal produced. The reference for "gaze changed nothing".
