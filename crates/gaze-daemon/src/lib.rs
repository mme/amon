...
//!
//! It is started on demand by any gaze command and then stays until something
//! kills it — including a newer gaze, which takes the socket over so an
...
fn bind(socket: &Path, version: &str) -> std::io::Result<UnixListener> {
    ...
    match ask_existing_daemon_to_exit(socket, version) {
        DaemonAtSocket::Newer => Err(std::io::Error::other(
            "a newer gaze daemon is already running",
        )),
...
fn serve(
    ...
    for line in reader.lines().map_while(Result::ok) {
        ...
        let response = match Request::parse(&line) {
            ...
            // Answer rather than hang up: a wrapper built against a newer gaze
