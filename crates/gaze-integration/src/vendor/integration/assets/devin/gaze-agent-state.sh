...
# Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/assets/devin/herdr-agent-state.sh
...
# GAZE_INTEGRATION_ID=devin
# GAZE_INTEGRATION_VERSION=2

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gaze-devin-hook.XXXXXX")" || exit 0
...
GAZE_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("GAZE_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("GAZE_HOOK_INPUT_FILE"))
