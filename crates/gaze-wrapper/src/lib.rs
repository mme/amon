//! Running an agent behind gaze.
//!
//! The contract is that the agent cannot tell. Its stdin, stdout, and terminal
//! size are exactly what they would be without gaze; gaze keeps a copy of the
...
use gaze_protocol::{env as protocol_env, AgentEntry, AgentState};
...
pub fn run(launch: Launch) -> std::io::Result<i32> {
    ...
    command.env(protocol_env::GAZE_ENV, protocol_env::GAZE_ENV_VALUE);
    ...
    // agent exits, instead of hanging because gaze still holds it open.
    drop(pty.slave);

    let agent_label = program.rsplit('/').next().unwrap_or(program).to_string();
    let agent = gaze_detect::parse_agent_label(&agent_label);
...
/// socket path, and `GAZE_AGENT_ID`. Process id plus launch time is unique per
