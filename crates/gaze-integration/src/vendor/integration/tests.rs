...
// Upstream: https://github.com/ogulcancelik/herdr/blob/514e4465ee33d4d81682d06ee2934483982a54ed/src/integration/tests.rs
...
fn enforce_agent_version_warns_when_binary_missing() {
    let requirement = AgentVersionRequirement {
        label: "kimi code",
        binary: "gaze-test-binary-that-does-not-exist",
        args: &["--version"],
        min_version: "0.14.0",
    };
...
fn integration_recommendation_installs_available_or_outdated_targets() {
    let mut recommendation = IntegrationRecommendation {
        ...
        command: "claude",
        available: false,
        path: PathBuf::from("/tmp/gaze-agent-state.sh"),
...
#[test]
fn install_omp_preserves_non_gaze_file_with_pi_install_name() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let ext_dir = home.join(".omp/agent/extensions");
...
fn outdated_integrations_treat_missing_version_marker_as_legacy() {
    ...
    fs::write(&extension_path, "// installed by gaze\n").unwrap();
    std::env::set_var("HOME", &home);
...
#[test]
fn uninstall_claude_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let claude_dir = home.join(".claude");
...
#[test]
fn uninstall_codex_removes_gaze_hooks_and_leaves_config_alone() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let codex_dir = home.join(".codex");
    ...
    for event in COPILOT_REMOVED_LIFECYCLE_HOOK_EVENTS {
        if let Some(entries) = settings["hooks"].get(event) {
            assert!(
                ...
                "expected gaze hooks.{event} entries to be removed"
            );
        }
    }
    assert!(settings["hooks"].get("sessionStart").is_none());
...
#[test]
fn uninstall_copilot_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let copilot_dir = home.join(".copilot");
...
#[test]
fn uninstall_devin_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let xdg_config = base.join("xdg");
    let devin_dir = xdg_config.join("devin");
...
#[test]
fn uninstall_droid_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let home = base.join("home");
    let droid_dir = home.join(".factory");
...
fn install_hermes_is_idempotent_for_enabled_entry() {
    ...
    assert_eq!(config.matches("gaze-agent-state").count(), 1);
...
fn install_hermes_is_idempotent_for_quoted_flat_plugin_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins:\n  - \"gaze-agent-state\" # installed by gaze\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn uninstall_hermes_removes_plugin_and_enabled_entry() {
    ...
    assert!(!config.contains("gaze-agent-state"));
...
fn uninstall_hermes_removes_flow_plugin_list_entry() {
    ...
    fs::write(
        hermes_dir.join("config.yaml"),
        "plugins: [other-plugin, gaze-agent-state]\n",
    )
    .unwrap();
    std::env::set_var("HOME", &home);
...
fn bundled_integration_assets_report_session_refs() {
    ...
    assert!(
        CODEX_HOOK_ASSET.contains("GAZE_HOOK_INPUT_FILE")
    ...
    assert!(KIMI_HOOK_ASSET.contains("source\": \"gaze:kimi"));
    ...
    assert!(DEVIN_HOOK_ASSET.contains("GAZE_DEVIN_LIST_JSON"));
    ...
    assert!(KILO_PLUGIN_ASSET.contains("SOURCE = \"gaze:kilo\""));
    ...
    assert!(QODERCLI_HOOK_ASSET.contains("GAZE_AGENT_ID"));
    ...
    assert!(CURSOR_HOOK_ASSET.contains("GAZE_INTEGRATION_ID=cursor"));
    ...
    assert!(MASTRACODE_HOOK_ASSET.contains("GAZE_INTEGRATION_ID=mastracode"));
    assert!(MASTRACODE_HOOK_ASSET.contains("GAZE_INTEGRATION_VERSION=2"));
    ...
    assert!(GROK_HOOK_ASSET.contains("GAZE_INTEGRATION_ID=grok"));
    ...
    assert!(GROK_HOOK_ASSET.contains("gaze:grok"));
...
#[test]
fn uninstall_qodercli_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qoder_dir = base.join(".qoder");
    fs::create_dir_all(&qoder_dir).unwrap();
    ...
    assert!(session_start[0]
        ...
        .is_some_and(|command| {
            command.starts_with("bash ")
                && command.contains("gaze-agent-state.sh")
...
#[test]
fn uninstall_cursor_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let cursor_dir = base.join(".cursor");
    fs::create_dir_all(&cursor_dir).unwrap();
...
fn install_mastracode_writes_hook_and_updates_hooks_json() {
    ...
    for (event, action) in MASTRACODE_HOOK_EVENTS {
        let entries = hooks.get(event).and_then(Value::as_array).unwrap();
        assert_eq!(entries.len(), 1, "{event} should have one Gaze hook");
...
fn install_grok_writes_hook_and_config() {
    ...
    assert!(command.contains("gaze-agent-state.sh"));
...
#[test]
fn uninstall_mastracode_removes_gaze_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let original_home = std::env::var_os("HOME");
    std::env::set_var("HOME", &base);
...
fn grok_v1_integration_status_is_current() {
    ...
    fs::create_dir_all(&grok_dir).unwrap();
    std::env::set_var(GROK_CONFIG_DIR_ENV_VAR, &grok_dir);
    // A real install writes both the hook script and hooks/gaze.json.
    install_grok().unwrap();
...
fn grok_status_reports_outdated_when_hook_config_missing_or_broken() {
    ...
    fs::write(
        &config_path,
        r#"{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"echo gaze-agent-state.sh"}]}]}}"#,
    )
    .unwrap();
    assert_eq!(grok_state(), IntegrationStatusKind::Outdated);
...
fn grok_dir_honors_grok_home_after_config_dir_seam() {
    ...
    // The gaze-level test seam still wins over GROK_HOME when set.
