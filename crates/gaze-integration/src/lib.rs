...
//!
//! `gaze install claude` writes a version-stamped hook script into the agent's
...
//!
//! The hook assets are the same too, with the herdr → gaze rename applied by
...
// `unused_imports`: the vendored module re-exports its full surface; gaze uses
