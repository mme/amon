...
Gaze mirrors herdr's manifest freshness behavior exactly: all agent manifests
...
`gazed` refreshes at daemon startup and every 24h thereafter (herdr itself only
...
If herdr.dev disappears or diverges, gaze degrades gracefully to its bundled
