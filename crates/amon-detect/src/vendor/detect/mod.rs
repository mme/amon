...
// Upstream: https://github.com/ogulcancelik/herdr/blob/095f1337d6502081658973ae3487dee8c6b34e1a/src/detect/mod.rs
...
pub fn session_identity_only_integration(source: &str, agent_label: &str) -> bool {
    matches!(
        (source, agent_label),
        ("amon:hermes", "hermes") | ("amon:qwen", "qwen") | ("amon:antigravity_cli", "agy")
    )
}
...
mod tests {
    ...
    fn session_identity_integrations_leave_state_to_screen_detection() {
        for (source, label, agent) in [
            ("amon:hermes", "hermes", Agent::Hermes),
            ("amon:qwen", "qwen", Agent::Qwen),
            ("amon:antigravity_cli", "agy", Agent::Antigravity),
        ] {
        ...
        let job = crate::platform::ForegroundJob {
            ...
            processes: vec![foreground_process(
                ...
                &[
                    ...
                    "-NoProfile",
                    "-File",
                    "C:\\Users\\amon\\Documents\\PowerShell\\Scripts\\claude.ps1",
                ],
            )],
        };
    ...
    // A plain shell pane launched with amon's injected prompt integration
    // must still classify as a shell, not an agent, even though its argv now
    // carries a -Command payload.
    #[test]
    fn identify_agent_in_job_ignores_amon_powershell_shell_integration_argv() {
        let job = crate::platform::ForegroundJob {
            process_group_id: 123,
            processes: vec![foreground_process(
                1,
    ...
    #[cfg(target_os = "linux")]
    #[test]
    #[ignore = "amon stubs out foreground process scanning; it identifies agents from argv"]
